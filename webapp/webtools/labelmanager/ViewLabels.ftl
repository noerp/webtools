      ...
      <#list labelsList as labelList>
        ...
        <#if showLabel == true>
          <tr <#if rowNum == "1">class="alternate-row"</#if>>
            <td>${rowNumber}</td>
            <td><a href="<@noerpUrl>UpdateLabel?sourceKey=${labelKey}&amp;sourceFileName=${label.fileName}&amp;sourceKeyComment=${label.labelKeyComment!}</@noerpUrl>" <#if previousKey == labelKey>class="submenutext"</#if>>${label.labelKey}</a></td>
            <td>${label.fileName}</td>
            <td><a href="<@noerpUrl>ViewReferences?sourceKey=${labelKey}&amp;labelFileName=${label.fileName}</@noerpUrl>">${uiLabelMap.WebtoolsLabelManagerReferences}</a></td>
            <#list localesFound as localeFound>
